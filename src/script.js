...
function model()
{
    ...
    this.filteredLocations = ko.computed(function(){
        if(this.query().trim() == '')
            ...
            var newArray = [];
            // hide all markers  (marker.setMap(null), marker.setVisable)
            for(var i=0; i<locations.length; i++){
                if(locations[i].title.toLowerCase() == this.query().trim().toLowerCase())
                {
                    newArray.push(locations[i]);
                }
            }
            // here you need to check if the query is equal to one of the titles inside locations. then if it's equal, add it to the newArray
            console.log(newArray);
            // show all markers that are in the newArray 
            return newArray; // return the filtered array
        }
    },this);
    this.titleClicked = function(loc)
    {
        console.log(loc);
